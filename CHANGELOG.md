...
### Added
- **Video subfolder organization** — videos now track their subfolder path relative to the connected root folder. The video database includes a new `subfolder` column indexed for fast filtering. A horizontal bookmark bar appears below the nav when subfolders are detected, displaying each subfolder as a clickable chip with video counts. The bookmark bar uses the same frosted glass transparency and blur as the nav, respects the auto-hide nav setting, and automatically hides during fullscreen video viewing. Users can quickly filter the library by subfolder (e.g., `subfolder:txt2img-grids`) via the bookmark bar or search syntax. The "All Videos" chip clears subfolder filters. A "Show subfolder filters" checkbox in the folder management panel toggles bookmark bar visibility (enabled by default). This addresses the primary pain point of managing massive video dumps with nested folder structures (ComfyUI output folders, AnimateDiff/Wan/HunyuanVideo/etc. subfolders). Backend: `subfolder` column added to videos table with index, `getSubfolders()` method returns unique subfolders with counts, search parser supports `subfolder:` filter, scanner passes root_path through to indexVideo for relative path calculation. Frontend: bookmark bar UI with chips, click handlers for filtering, auto-refresh on load, theme-aware styling matching nav transparency/blur, auto-hide behavior, folder panel toggle. API: `GET /api/videos/subfolders` endpoint. (`app/server/video-database.js`, `app/server/video-scanner.js`, `app/server/index.js`, `app/public/modules/api.js`, `app/public/video-modules/main.js`, `app/views/videos.ejs`, `app/public/videos.css`, `app/public/global.css`)
