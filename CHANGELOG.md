...
### Added
- **Video subfolder organization** — videos now track their subfolder path relative to the connected root folder. The video database includes a new `subfolder` column indexed for fast filtering, and the search parser supports `subfolder:` filters. A horizontal bookmark bar appears below the video nav with browser-style connected-folder chips and counts. Clicking a root folder chip filters to every indexed video under that folder; right-clicking opens a subfolder dropdown for precise drill-down. The "All Videos" chip clears folder/subfolder filters. The bookmark bar uses the same frosted glass transparency and blur as the nav, respects the auto-hide nav setting, and automatically hides during fullscreen video viewing. This addresses the primary pain point of managing massive video dumps with nested folder structures (ComfyUI output folders, AnimateDiff/Wan/HunyuanVideo/etc. subfolders). Backend: `subfolder` column added to videos table with index, scanner passes `root_path` through to `indexVideo()` for relative path calculation, and video folder bookmark data is exposed through the API. Frontend: bookmark bar UI with parent folder chips, right-click subfolder menus, click handlers for filtering, auto-refresh on load, theme-aware styling matching nav transparency/blur, and auto-hide behavior. API: `GET /api/videos/subfolders` and `GET /api/videos/folder-bookmarks` endpoints. (`app/server/video-database.js`, `app/server/video-scanner.js`, `app/server/index.js`, `app/public/modules/api.js`, `app/public/video-modules/main.js`, `app/views/videos.ejs`, `app/public/videos.css`, `app/public/global.css`)
- **Image tab parity refactor** — the image tab has been brought up to feature parity with the video tab, adopting its polished UI patterns. The image gallery now supports a masonry grid layout (Contain / Cover / Masonry) with per-image native aspect ratios, replacing the single flex-wrap grid. A bookmark bar with browser-style folder chips and right-click subfolder dropdown menus provides folder-based navigation mirroring the video tab. Grid layout, image limit, and viewer panel settings are now in a reorganized settings sidebar. The image database gained a `recursive` flag on the folders table, a `root_path:` search filter, and a `getFolderBookmarks()` query backed by a new `GET /api/folders/bookmarks` endpoint. Folder connection now includes an "Include subfolders" checkbox. (`app/public/global.css`, `app/public/videos.css`, `app/views/index.ejs`, `app/public/modules/app.js`, `app/public/modules/navbar.js`, `app/public/modules/card.js`, `app/public/modules/api.js`, `app/server/image-database.js`, `app/server/index.js`)

### Changed
- **Image tab hover headers** — the "On Hover" card header mode now uses the same frosted glass styling as the video tab: translucent blue-white tint (`rgba(176,206,240,0.16)`), backdrop blur, subtle border/shadow, and white icon coloring with text shadow. (`app/public/global.css`)
- **CSS deduplication** — shared styles (bookmark bar, chips, folder menus, nav checkbox toggle, grid layout) were extracted from `videos.css` into `global.css` so both tabs use the same rules. Video-specific masonry and flex grid styles remain in `videos.css` with their own CSS variables. (`app/public/global.css`, `app/public/videos.css`)
- **Slider performance** — zoom and aspect ratio sliders in the image settings sidebar now debounce CSS variable updates to 60ms, fixing jank on large libraries. (`app/public/modules/navbar.js`)
- **Video settings ergonomics** — merged the separate Card Size and Grid Layout controls into a single Grid Layout section with `Contain`, `Cover`, and `Masonry` presentation choices. The card width control is now labeled `Scale`, the aspect slider hides automatically for Masonry because masonry uses each video's native aspect, and the video limit slider now supports `0` as `No limit` with a 5000-video upper bound for capped mode. Added a Video Viewer setting to start the expanded viewer with the info panel hidden, and the settings sidebar now closes when users click outside it. Backend video search treats `limit=0` as uncapped instead of returning zero rows. (`app/public/video-modules/main.js`, `app/server/video-database.js`, `app/CHANGELOG.md`)
- **Video folder bookmark bar refinement** — replaced the one-chip-per-subfolder bookmark bar with browser-style parent folder chips. Left-clicking a connected root folder filters to every indexed video under that folder, while right-clicking opens a dropdown of indexed subfolders for precise drill-down. The previous folder-panel "Show subfolder filters" control is now a main-nav bookmark-bar visibility toggle, and the folder panel now uses "Include subfolders" for its intended indexing behavior when connecting or re-scanning video folders, including per-folder toggles for existing connections. Video folders persist a `recursive` flag, the scanner honors it, search now supports quoted `root_path:"..."` filters for paths with spaces, and a new `GET /api/videos/folder-bookmarks` endpoint returns root-folder bookmark data with nested subfolder counts. (`app/server/video-database.js`, `app/server/index.js`, `app/public/modules/api.js`, `app/public/video-modules/main.js`, `app/views/videos.ejs`, `app/public/videos.css`, `app/public/global.css`)
...
### Added
- **Video player controls** — inline fullscreen video viewer now has a custom seek bar with buffered/progress state, elapsed/total time, global volume slider, mute toggle, frame capture (`S`), frame step backward/forward (`,` / `.` using indexed FPS with 30fps fallback), and single-click play/pause with double-click/Esc close. Play-locked grid cards now expose an independent mute button while locked. The pop-out video viewer has been redesigned around the same controls with pan/zoom, frame capture, frame step, custom seek/volume controls, and matching shortcuts. (`app/public/video-modules/main.js`, `app/views/video-viewer.ejs`, `app/public/global.css`, `app/public/videos.css`)
